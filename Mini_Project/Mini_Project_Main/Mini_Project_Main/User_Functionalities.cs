            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    SqlCommand command = new SqlCommand(
                        ...
                        "FROM Trains WHERE Source = @Source AND Destination = @Destination AND TrainDate = @TrainDate AND IsActive = 1", connection);
                    ...
                    SqlCommand priceCommand = new SqlCommand(
                        "SELECT TicketPrice1A, TicketPrice2A, TicketPrice3A, TicketPriceFirstClass, TicketPriceSleeper FROM Trains WHERE TrainNo = @TrainNo", connection);
                    ...
                    if (priceReader.HasRows)
                    {
                        ...
                        {
                            case "1A":
                                ticketPrice = priceReader.GetDecimal(0);
                                break;
                            case "2A":
                                ticketPrice = priceReader.GetDecimal(1);
                                break;
                            case "3A":
                                ticketPrice = priceReader.GetDecimal(2);
                                break;
                            case "First Class":
                                ticketPrice = priceReader.GetDecimal(3);
                                break;
                            case "Sleeper":
                                ticketPrice = priceReader.GetDecimal(4);
                                break;
                            default:
                    ...
                    SqlCommand bookCommand = new SqlCommand(
                        "INSERT INTO Bookings (TrainNo, PassengerName, Class, BerthsBooked, JourneyDate, Status, TotalCost, TrainDate, TrainTime) " +
                        "VALUES (@TrainNo, @PassengerName, @Class, @BerthsBooked, @JourneyDate, 'Booked', @TotalCost, @TrainDate, @TrainTime)", connection);
